...
export default function Piece(props: {
  ...
  const pieceData = Array.from({ length: numRows }, (_, row) =>
    Array.from(
      { length: numCols },
      (_, col) => props.pieceData[row][col] ?? BlockType.EMPTY
    )
  );
  ...
  return (
    <div
      ...
      className={c(s.Piece, props.className)}
      style={{
        ...props.additionalStyles,
        gap: `${100 / (numRows / boardGridGapSize + numRows - 1)}% ${
          100 / (numCols / boardGridGapSize + numCols - 1)
        }%`,
        gridTemplate: `repeat(${numRows}, 1fr) / repeat(${numCols}, 1fr)`,
        height: props.blockSize
          ? `${
              props.blockSize * (numRows + (numRows - 1) * boardGridGapSize)
            }px`
          : `${
              (100 * (numRows + (numRows - 1) * boardGridGapSize)) /
              (gridSize + (gridSize - 1) * boardGridGapSize)
            }%`,
        width: props.blockSize
          ? `${
              props.blockSize * (numCols + (numCols - 1) * boardGridGapSize)
            }px`
          : `${
              (100 * (numCols + (numCols - 1) * boardGridGapSize)) /
              (gridSize + (gridSize - 1) * boardGridGapSize)
            }%`,
      }}
    >
      {pieceData.map((row, rowNum) =>
